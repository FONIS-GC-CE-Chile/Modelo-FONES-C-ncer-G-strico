...
p.HD <- 0.02  # probability to die when healthy # This is time-dependant (m.dead)
...
p.pH1p <- p.H1p  # probability to become stage 1 preclinical when healthy
p.p1p1c <- p.1p1c * t.diag  # probability to become stage 1 clinical when stage 1 preclinical. 
p.p1p2p <- p.1p2p  # probability to become stage 2 preclinical when stage 1 preclinical
p.p2p2c <- p.2p2c * t.diag  # probability to become stage 2 clinical when stage 2 preclinical
p.p2p3p <- p.2p3p   # probability to become stage 3 preclinical when stage 2 preclinical
p.p3p3c <- p.3p3c * t.diag  # probability to become stage 3 clinical when stage 3 preclinical
p.p3p4p <- p.3p4p  # probability to become stage 4 preclinical when stage 3 preclinical
p.p4p4c <- p.4p4c * t.diag  # probability to become stage 4 clinical when stage 4 preclinical
p.p1cH <- p.1cH  # probability to become healthy when stage 1 clinical (revisar si es pertinente)
p.p2cH <- p.2cH  # probability to become healthy when stage 2 clinical (revisar si es pertinente)
p.p3cH <- p.3cH  # probability to become healthy when stage 3 clinical (revisar si es pertinente)
p.p4cH <- p.4cH  # probability to become healthy when stage 4 clinical (revisar si es pertinente)

#Hazard ratio (conversar Tania)
...
# Costs and utilities. Requiere dferenciar año de diagnostico (t1) y años de seguimiento (t2-t5)
